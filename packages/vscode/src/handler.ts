...
export class MessageHandler {
  ...
  /**
   * jj workspace 聚合的有效值：webview 明確帶 includeJj 時以其為準，
   * 否則回退到 `spek.aggregateJjWorkspaces` 設定（預設 true）。
   */
  private jjEnabled(includeJj?: boolean): boolean {
    if (includeJj !== undefined) return includeJj;
    return vscode.workspace
      .getConfiguration("spek")
      .get<boolean>("aggregateJjWorkspaces", true);
  }
