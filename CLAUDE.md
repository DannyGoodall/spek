...
- 設定 `spek.aggregateJjWorkspaces`（boolean, default false，實驗性 opt-in）：是否將 jj workspace 納入聚合（與 git worktree 聚合獨立）；handler / tree-provider / panel watcher 讀此設定傳 `includeJj`。jj workspace 來源在 sidebar 以 `jj:<name>` 標示，`@` 正在編輯者標「✎ editing」
